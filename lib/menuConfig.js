...
export const FEATURED_ITEMS = ['Large Coffee', 'BACON & EGG ROLL', 'BREKKY WRAP'];
...
export function pickFeatured(curatedCategories = [], count = 3) {
  ...
  let picked = all.slice(0, count);
  if (FEATURED_ITEMS.length) {
    const chosen = FEATURED_ITEMS.map((name) =>
      all.find((it) => norm(it.name) === norm(name)),
    ).filter(Boolean);
    if (chosen.length) picked = chosen.slice(0, count);
  }
  // Renumber sequentially for display — picked items keep their catalog
  // category order, not the "No." they had within their own category.
  return picked.map((it, i) => ({ ...it, number: String(i + 1).padStart(2, '0') }));
}
